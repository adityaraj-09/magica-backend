...
<Steps>
  ...
  <Step title="Submit a turn">
    `POST /api/v1/completions` with `{ "text": "…" }` — omit `chatId` to start a new chat, or pass it to continue. Add `image_urls` to attach public images. The HTTP call returns immediately with `runId`.
  </Step>
